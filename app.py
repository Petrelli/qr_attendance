...
# ====== 可自行修改 ======
TEACHER_PIN = "123456"   # 教师端口令，默认 123456，建议改掉
DEFAULT_SIGNIN_MINUTES = 5
TIMEZONE = timezone.utc   # 如需改成北京时间，可换成 timezone(timedelta(hours=8))
# =======================
  ...
  {% if not authenticated %}
    <div class="card">
      ...
      <form method="post" action="{{ url_for('teacher_login') }}">
        <input type="password" name="pin" placeholder="输入教师端 PIN" required>
        <button type="submit">登录</button>
      </form>
      <p>默认 PIN 为 <code>123456</code>，建议在 <code>app.py</code> 里修改。</p>
    </div>
  {% else %}
    <div class="card">
      <h2>上传学生名单</h2>
      <form method="post" action="{{ url_for('upload_students') }}" enctype="multipart/form-data">
        ...
        <button type="submit">上传并覆盖 students.txt</button>
      </form>
      <p>文件要求：每行一个 student ID。当前名单人数：<strong>{{ student_count }}</strong></p>
    </div>

    <div class="card">
      <h2>创建签到二维码</h2>
      <form method="post" action="{{ url_for('create_session') }}">
        <div><input type="text" name="course_name" placeholder="课程名称，如 Communication Networks" required style="width: 320px;"></div>
        <div><input type="number" name="minutes" min="1" max="120" value="{{ default_minutes }}" required> 分钟有效</div>
        <button type="submit">生成本节课二维码</button>
      </form>
    </div>
    ...
    <div class="card">
      ...
      <table>
        <thead>
          <tr>
            <th>ID</th><th>课程</th><th>创建时间</th><th>截止时间</th><th>状态</th><th>操作</th>
          </tr>
        </thead>
        <tbody>
        {% for s in sessions %}
          <tr>
            ...
            <td>{{ s['created_at'] }}</td>
            <td>{{ s['expires_at'] }}</td>
            <td>{% if s['is_active'] %}启用{% else %}关闭{% endif %}</td>
            <td>
              <a href="{{ url_for('view_qr', session_id=s['id']) }}">查看二维码</a> |
              <a href="{{ url_for('view_records', session_id=s['id']) }}">签到结果</a>
            </td>
          </tr>
        {% endfor %}
        </tbody>
...
<body>
  <div class="card">
    ...
    <p>签到场次 ID：{{ session['id'] }}</p>
    <p>有效截止时间：{{ session['expires_at'] }}</p>
    <p><img src="{{ qr_data }}" alt="签到二维码"></p>
    <p>签到链接：<a href="{{ sign_url }}">{{ sign_url }}</a></p>
    <p><a href="{{ url_for('view_records', session_id=session['id']) }}">查看实时签到结果</a></p>
    <p><a href="{{ url_for('admin') }}">返回教师端</a></p>
  </div>
</body>
</html>
"""
...
<body>
  <div class="card">
    <h1>学生签到</h1>
    <p>课程：{{ session['course_name'] }}</p>
    <p>签到截止：{{ session['expires_at'] }}</p>
    ...
    {% if not closed and not success %}
      <form method="post" action="{{ url_for('submit_signin') }}">
        ...
        <input type="text" name="student_id" placeholder="请输入你的 student ID" required>
...
<body>
  <div class="card">
    <h1>{{ session['course_name'] }} - 签到结果</h1>
    <p>场次 ID：{{ session['id'] }}</p>
    <p>签到截止：{{ session['expires_at'] }}</p>
    <p>已签到人数：<strong>{{ records|length }}</strong></p>
    <p>
      <a href="{{ url_for('export_records', session_id=session['id']) }}">导出 CSV</a> |
      <a href="{{ url_for('view_qr', session_id=session['id']) }}">返回二维码</a> |
      <a href="{{ url_for('admin') }}">返回教师端</a>
    </p>
  </div>
...
def upload_students():
    ...
    with open(STUDENT_FILE, "w", encoding="utf-8") as out:
        out.write("\n".join(lines) + ("\n" if lines else ""))
    flash(f"上传成功，共导入 {len(lines)} 个 student ID。")
    return redirect(url_for("admin"))
...
def signin():
    ...
    if not session:
        return render_template_string(SIGNIN_TEMPLATE, session={"course_name": "未知课程", "expires_at": "-"}, token=token, message="无效二维码。", success=False, closed=True)
    ...
    return render_template_string(
        ...
        session=session,
        token=token,
        message="签到通道已关闭。" if expired else None,
        success=False,
        closed=expired,
    )
...
def submit_signin():
    ...
    if (not session["is_active"]) or (datetime.now(TIMEZONE) > parse_dt(session["expires_at"])):
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="签到已截止。", success=False, closed=True)

    if not student_id:
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="student ID 不能为空。", success=False, closed=False)

    if student_id not in load_students():
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="student ID 不在名单中，签到失败。", success=False, closed=False)
    ...
    if existing_student:
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="该 student ID 已签到，不能重复签到。", success=False, closed=False)

    if not device_id:
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="设备标识缺失，请使用正常浏览器重新扫码。", success=False, closed=False)
    ...
    if existing_device:
        return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="该手机已经签到过一次，不能代签。", success=False, closed=False)
    ...
    db.commit()
    return render_template_string(SIGNIN_TEMPLATE, session=session, token=token, message="签到成功。", success=True, closed=True)
